...
using namespace std;
/**
 * @brief node used to build huffman tree
 * 
 */
struct huffman_node {
	...
	/**
	 * @brief as huffman tree is binary tree so every node may have 2 children
	...
	/**
	 * @brief constructer to set children to null as initial as may be the node don't have any children
...
/**
 * @brief parent class for classes used in compression & deccompression
 * 
 */
class huffman
{
protected:
	/**
	 * @brief as we have 128 characters in the Ascii Table
	...
	/**
	 * @brief original file ----(compression)--> //  encoded file  // ----(Decompression)--> Decoded file
	 * 
	 */
	 
	/**
	 * @brief in_file is file handler for original file in case of Compression
		is file handler for encoded file in case of DeCompression
	*/
	fstream in_file;

	/**
	 * @brief in_file is file handler for Encoded file in case of Compression
...
public:
	 ...
	 * @brief constructor to initalize the original file and Ecodeded file in case of Encoding
				or to initalize the Encodeded file and Decoded file in case of Decoding
